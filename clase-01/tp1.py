...
def apply_yiq_transformation(image, alpha=1, beta=1):
    ...
    return yiq_to_rgb(yiq_prime_image).astype(np.uint8)


def main():
    file_name = 'imageio:astronaut.png'  # Cambiar esto para que se cargue una imagen

    alpha = .1
    beta = .1

    img = imageio.imread(file_name)
    m_img = apply_yiq_transformation(img, alpha, beta)

    f_name = file_name.split('/')[-1]

    imageio.imwrite(f'modified_{f_name}', m_img)
